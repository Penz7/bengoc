...
function toggleGift() {
  ...
  const paper = document.querySelector('.boxmail');
...
function flipCard(cardIndex) {
  ...
  cards[cardIndex].classList.add('flip');

  if (cards[cardIndex].classList.contains('gold')) {
    var randomIndex = Math.floor(Math.random() * cards.length);
    while (randomIndex === cardIndex) {
      randomIndex = Math.floor(Math.random() * cards.length);
    }
    cards[cardIndex].classList.remove('flip', 'gold');
    cards[randomIndex].classList.add('gold');
  }
}


document.getElementById("special").addEventListener("mouseover", function (event) {
  var rect = event.target.getBoundingClientRect();
  var newX = Math.random(10) * (900 - rect.width);
  var newY = Math.random(10) * (500 - rect.height);
